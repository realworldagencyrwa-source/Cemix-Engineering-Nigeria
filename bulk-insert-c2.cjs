...
async function insertProducts() {
  console.log(`Attempting to insert ${products.length} missing C-2 Series products...`);

  // Try inserting in batches to avoid timeout
  const batchSize = 5;
  let successCount = 0;
  let errorCount = 0;
  ...
  for (let i = 0; i < products.length; i += batchSize) {
    ...
    console.log(`\nInserting batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(products.length / batchSize)} (${batch.length} products)...`);
  ...
  if (!countError) {
    console.log(`\nTotal C-2 Series products in database: ${allC2.length}`);
  }
}
